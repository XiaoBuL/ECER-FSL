...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, return_all_tokens=None, mask=None):
        ...
        for layer in range(len(self.layers)):
            ...
            C = x.shape[2]
            if layer == 0:
                self.layer2_out = x.reshape(B,C,int(sqrt(HW)),int(sqrt(HW)))
            elif layer == 1:
                self.layer3_out = x.reshape(B,C,int(sqrt(HW)),int(sqrt(HW)))
            elif layer == 2 :
                self.layer4_out = x.reshape(B,C,int(sqrt(HW)),int(sqrt(HW)))
