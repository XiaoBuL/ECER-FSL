...
IMAGE_PATH = 'path/to/your/cub/'
SPLIT_PATH =  'path/to/your/cub/split'
...
class CUB(Dataset):
    ...
    def __init__(self, setname, args, augment=False):
        ...
        if self.use_im_cache:
            if not osp.exists(cache_path):
                ...
                resize_ = 0 if im_size < 0 else transforms.Resize(im_size) # Warn
    ...
    def __getitem__(self, i):
        ...
        class_name = filename.split('0')[0][:-1]
        captions_name = []
        captions_name.append(self.wnids[label].split('.')[1])
        captions_name.append(filename)
        if self.use_im_cache:
            image = self.transform(data)
        else:
            image = self.transform(Image.open(data).convert('RGB'))
        return image, label,captions_name            
